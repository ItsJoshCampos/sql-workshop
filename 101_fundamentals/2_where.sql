...
FROM 
    patient 
WHERE sex = 'M' 
    AND reason_for_visit ='High blood pressure';
...
WHERE 
    sex = 'F' 
    AND (birth_dt > '1990-01-01' 
        AND birth_dt < '2000-01-01');
...
-- 3A. Retrieve information on patients who are not associated with facility_id '4' and '5'.
SELECT * 
FROM 
    patient 
WHERE 
    facility_id NOT IN (4, 5);

-- 3B. Retrieve information on patients who are associated with facility_id '1' and '2'.
SELECT * 
FROM 
    patient 
WHERE 
...
FROM 
    patient 
WHERE 
    birth_dt BETWEEN '1985-01-01' AND '1995-12-31' 
    AND reason_for_visit LIKE '%pain%';
...
WHERE 
    sex = 'F' 
    AND (last_name = 'Smith' OR last_name = 'Jones') 
    AND (reason_for_visit = 'Allergic reaction');
