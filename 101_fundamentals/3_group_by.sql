...
-- 0. Find the number of patients for each facility.
...
-- 1. Identify facilities with more than 24000 patients.
SELECT facility_id, COUNT(patient_id) AS patient_count 
FROM patient 
GROUP BY facility_id 
HAVING COUNT(patient_id) > 24000;

-- 2. Determine the number of male and female patients in each facility.
...
-- 3. List reasons for visit that have been cited by more than 400 patients.
SELECT reason_for_visit, COUNT(patient_id) AS number_of_occurrences 
FROM patient 
GROUP BY reason_for_visit 
HAVING COUNT(patient_id) > 400;

-- 4. Identify facilities that have treated more female patients than male patients.
SELECT facility_id, 
...
-- 5. Retrieve the number of patients with the same birth date.
SELECT birth_dt, COUNT(patient_id) AS patient_count 
FROM patient 
GROUP BY birth_dt 
HAVING COUNT(patient_id) > 2;

-- 6. Find reasons for visit from facilities with more than 100 different patients citing the same reason.
